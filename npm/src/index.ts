...
export class Binary {
  ...
  protected constructor(
    ...
    try {
      this.urls = urls.map((path) =>
        typeof path === "string" ? new URL(path) : path
      );
    } catch (e) {
      errors.push(e);
    }
  ...
  static async create(
    ...
    let installDir =
      destination ?? (await searchPath(name)) ?? Binary.DEFAULT_INSTALL_DIR;
    const bin = new Binary(name, path, installDir);
